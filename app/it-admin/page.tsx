...
export default function ITAdminPage() {
  ...
  const [credStatuses, setCredStatuses] = useState<
  ...
  useEffect(() => {
    fetch("/api/users")
      .then((r) => r.json())
      .then(async (data) => {
        const agentUsers = (data.data || []).filter(
          (u: Agent & { role: string }) => u.role === "agent"
        );
        setAgents(agentUsers);

        // Check credential status for each
        ...
        await Promise.all(
          agentUsers.map(async (agent: Agent) => {
            try {
              ...
              statuses[agent.id] = !!(cred.data && cred.data.credentialsComplete);
            } catch {
        ...
        <TableBody>
          {agents.map((agent) => (
            <TableRow key={agent.id}>
              ...
              <TableCell>
                <Link href={`/it-admin/agents/${agent.id}/credentials`}>
